...
/* self-defined job util */

int isjobsfull(struct job_t *jobs);
...
void eval(char *cmdline)
{
    ...
    if (!bg && (fgpid(jobs) > 0))
    {
        fprintf(stderr, "Foreground job is running");
        return;
    }

    if (isjobsfull(jobs))
    {
        fprintf(stderr, "There are too many jobs.");
        return;
    }
    ...
    if (bg)
    {
        fprintf(stdout, "[%d] (%d) %s", jobp->jid, jobp->pid, jobp->cmdline);
    }
...
/*
 * hasemptyjob - check if there is any empty slot for new job
 */
int isjobsfull(struct job_t *jobs)
{
...
pid_t Waitpid(pid_t pid, int *wstatus, int options)
{
    ...
    if ((_pid = waitpid(pid, wstatus, options)) < 0 && errno != ECHILD)
    {
